...
use app\models\search\DevaOwnerSearch;
...
class AdminAppController extends Controller
{
    ...
    public function actionItem($type, $id = null)
    {
        if ($type == 'role') {
            ...
            throw new UserException('Wrong Auth Item type, please contact your supervisor.');
        }
        if ($id) {
        ...
        $post_data['AuthItem']['data'] = json_encode(Yii::$app->getRequest()->post('AuthItem')['data']);
        if ($model->load($post_data) && $model->save()) {
            return $this->redirect(['/admin/'.$type.'/view', 'id' => $model->name]);
        }

        throw new UserException('Error managing '.$type.', please contact your supervisor.');
    }
    ...
    public function actionMenuEasyOrder()
    {
        if ($new_order = Yii::$app->getRequest()->post('new_order')) {
            ...
            foreach ($new_order as $parent_index => $parent_item) {
                ...
                $parent_model->order = $parent_index;
        ...
        return $this->render('//admin/menu/easy_order', [
            'menuItems' => $items,
            'pageName' => $page_name,
            'icon' => $icon,
        ]);
